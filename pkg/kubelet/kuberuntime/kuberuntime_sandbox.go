...
func (m *kubeGenericRuntimeManager) restorePodSandbox(ctx context.Context, pod *v1.Pod, attempt uint32) (string, string, error) {
	...
	logger.V(2).Info("Restoring pod sandbox from checkpoint", "pod", klog.KObj(pod), "checkpointLocation", *pod.Spec.RestoreFrom, "runtimeHandler", runtimeHandler)
	...
	if err != nil {
		...
		logger.Error(err, "Failed to restore pod sandbox from checkpoint", "pod", klog.KObj(pod), "checkpointLocation", *pod.Spec.RestoreFrom)
		return "", message, err
	}
	...
	logger.V(2).Info("Successfully restored pod sandbox from checkpoint", "pod", klog.KObj(pod), "podSandboxID", podSandBoxID, "checkpointLocation", *pod.Spec.RestoreFrom)
